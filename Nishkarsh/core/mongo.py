...
from Nishkarsh import config, logger, userbot
...
class MongoDB:
    def __init__(self):
        ...
        """
        self.mongo = AsyncMongoClient(config.MONGO_URL, serverSelectionTimeoutMS=30000)
    ...
    async def get_admins(self, chat_id: int, reload: bool = False) -> list[int]:
        from Nishkarsh.helpers._admins import reload_admins
    ...
    async def _get_auth(self, chat_id: int) -> set[int]:
        if chat_id not in self.auth:
            try:
                ...
                logger.error(f"Failed to fetch auth for chat {chat_id}: {e}")
                return set()
    ...
    async def add_auth(self, chat_id: int, user_id: int) -> None:
        ...
        if user_id not in users:
            try:
                ...
                logger.error(f"Failed to add auth for chat {chat_id}: {e}")
    ...
    async def rm_auth(self, chat_id: int, user_id: int) -> None:
        ...
        if user_id in users:
            try:
                ...
                logger.error(f"Failed to remove auth for chat {chat_id}: {e}")
    ...
    async def get_lang(self, chat_id: int) -> str:
        if chat_id not in self.lang:
            try:
                ...
                logger.error(f"Failed to fetch language for chat {chat_id}: {e}")
                return "en"
    ...
    # SONG METHODS
    async def save_song(self, song_id: str, data: dict) -> None:
        await self.songsdb.update_one({"_id": song_id}, {"$set": data}, upsert=True)

    async def get_song(self, song_id: str) -> dict | None:
        return await self.songsdb.find_one({"_id": song_id})
